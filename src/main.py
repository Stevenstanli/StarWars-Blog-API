...
"""
# pipenv shell (para abrir entorno virtual de python y poder gestionar las versiones del editor y las importaciones necesarias, no se puede llevar esto a cabo si no hay un entorno virtual en proceso)
...
def get_all_user():
    ...
    all_user = list(map(lambda user: user.serialize2(),all_user))
...
def profile():
    if request.method == 'GET':
        ...
        return jsonify({"Mensaje": "Acceso a espacio privado", "usuario": token}), 200
...
def get_planet(uid):
    ...
    if not pla:
        return jsonify({"Mensaje":"Planeta no encontrado"})
    pla = pla.serialize()
    return jsonify(pla)
...
def get_vehicle(uid):
    ...
    if not vehicle:
        return jsonify({"Mensaje":"Vehiculo no encontrado"})
...
def all_favorites(user_id):
    ...
    if not user:
        return jsonify({'Mensaje': "No existe un usuario con ese ID"})
    ...
    if not character_uid and not planet_uid and not vehicle_uid:
        return jsonify({'Mensaje': "Debes de especificar un ID de un personaje, planeta o vehiculo"})
    if character_uid:
        character = Characters.query.get(character_uid)
        if not character:
            return jsonify({'Mensaje': "El ID del personaje no es valido"})
    ...
    if planet_uid:
        ...
        if not planet:
            return jsonify({'Mensaje': "El ID del planeta no es valido"})
    ...
    if vehicle_uid:
        ...
        if not vehicle:
            return jsonify({'Mensaje': "El ID del vehiculo no es valido"})
...
def delete_favorite(user_uid):
    ...
    return jsonify({"Mensaje": "Favorito eliminado exitosamente"}), 201
